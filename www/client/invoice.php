...
if (!isset($_COOKIE['session_id']) || empty($_COOKIE['session_id'])) {
    header('Location: return.php?returnCode=6');
}

if (isset($_GET['id']) && !isblank($_GET['id'])) {
    $invoice_id = $_GET['id'];
}
...
';
$stm = $pdo->prepare($sql);
$stm->execute([$_COOKIE['session_id'], $invoice_id]);
$invoice = $stm->fetch();

if (count($invoice) == 0) {
    header('Location: return.php?returnCode=5');
}

$sql = '
    SELECT users.name, users.address, users.zip, users.city, users.phonenr
    FROM users
    INNER JOIN sessions
    ...
    $invoiceList .= '
        <div class="dropdown" id="i'.$invoiceCount.'">
            <div class="dropdownHeader" onclick="dropdownToggle('.$invoiceCount.')">
                <p>Invoice #'.$invoice['id'].'<i>Placed on: '.$invoice['created_date'].'</i><span class="arrow up"></span></p>
            </div>
